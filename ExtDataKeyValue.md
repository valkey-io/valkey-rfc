...
# Title
External data storage (keys + values)
...
Both keys and values are about to be stored externally - option to store values only externally is proposed in another RFC (#26).
...
1) Keys and values are stored neither in Valkey dict, nor in any Valkey directly controlled memory space.
2) Both keys and values externally stored are interacted via module - exact implementation of how this is done is up to module creator.
...
* Passing key/value storage<->client should be made in a way to prevent OOM during loading large data to limited memory.

### GET flow
1) Client requests a key.
2) Valkey dict is missing the key.
3) Key is checked against the external storage, if it's out OR requested without flag (VF1) - end of story.
4*) It's in - value is requested from external storage, returned to client.
...
3*) Key and value are put to external storage.
4) Key-value pair is deleted from Valkey dict, if the key was in memory before current SET.
5) Timestamp in terms of external storage on successful key-value write is returned to core.
...
1*) On expire SET key-value to external storage instead of deleting completely.

### DEL flow
1) On expire\DEL mark key-value pair as to be deleted from external storage.

### SCAN/KEYS flow
1*) Getting keys without flag (VF3) is similar to existing; with flag requires iterator module API.
...
### DBSIZE flow
1) Requires key-value dynamic counter for each database to quickly return it without recounting on each request.
...
2) DUMP command to save current state of external storage, LOAD command to restore this state later.

### External storage key-value pair lifecycle
1) Key-value pair is stored somehow in external storage (by size, directly or expiration).
...
- The maximum living time since last read|write for key-value pair external storage could be set in Valkey config (VC2), it's existence is not guaranteed after that.
- Key-value pair is not supposed to be returned back to memory from external storage on any occasion - this could be introduced separately later with some config options if that's really a latency problem for somebody in some usecases (doing that naively and by default could lead to ping-pong between memory and external storage slowing down database in whole for most or even all keys).
...
(VC3) extdata-store-by-expire[bool]: no=[default]don't expire data to external storage

### Keyspace notifications
1. Key is directly set to external storage.
2. Key is expired to external storage.
3. Key is explicitly deleted from external storage.
4. Key is deleted because appropriate value was freed from external storage (see GET flow).
...
3. ValkeyModuleExternalStorageMethods: a struct with following attributes:
    ...
    /* The callback function called when `DEBUG BLOCK` command is called in this storage. */
    ValkeyModuleExternalStorageSetReadonlyFunc set_readonly;

    /* The callback function called when `DEBUG UNBLOCK` command is called in this storage. */
...
### Debug mechanism
1. EXT_DATA DEBUG SET: set a key-value pair directly to external storage
2. EXT_DATA DEBUG DEL: delete a key-value pair directly from external storage
